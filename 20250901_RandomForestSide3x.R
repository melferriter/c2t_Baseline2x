...
fieldpoints_clean <- fieldpoints %>%
  mutate(
    Side3x = str_trim(Side3x),                 # remove spaces
    Side3x = str_replace_all(Side3x, "[\r\n]", "")  # remove carriage returns / newlines
  ) %>%
  dplyr::rename(treeID = Side3x) %>%
...
crowns$sequoia_group
crowns_final <- crowns_final %>%
  filter(sequoia_group == "Giant sequoia")
...
ggsave(
  filename = "VariableImportanc3xSide.png",  # file name
...
cv_summary <- cv_preds %>%
  summarise(
    R2   = yardstick::rsq_vec(obs_cm, pred_cm),
    RMSE = yardstick::rmse_vec(obs_cm, pred_cm),
    MAE  = yardstick::mae_vec(obs_cm, pred_cm),
    Bias = mean(pred_cm - obs_cm)
  )

cv_summary

cv_summary_overall <- cv_summary %>%
  summarise(across(c(R2, RMSE, MAE), mean))
# ============================
# 5. Diagnostics Plots
# ============================
...
ggsave(
  filename = "Scatter3xSide.png",  # file name
  plot = last_plot(),                          # saves the most recent ggplot
  path = "C:/Users/User/Desktop",      # <-- change to your folder
  width = 7, height = 5,                       # adjust for paper layout
  dpi = 1200                                    # high resolution for publication
...
ggsave(
  filename = "Residual3xSide1.png",  # file name
  plot = last_plot(),                          # saves the most recent ggplot
  path = "C:/Users/User/Desktop",      # <-- change to your folder
  width = 7, height = 5,                       # adjust for paper layout
  dpi = 1200                                    # high resolution for publication
...
ggsave(
  filename = "dist3xSide.png",  # file name
  plot = last_plot(),                          # saves the most recent ggplot
  path = "C:/Users/User/Desktop",      # <-- change to your folder
  width = 7, height = 5,                       # adjust for paper layout
  dpi = 1200                                    # high resolution for publication
