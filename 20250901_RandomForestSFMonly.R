...
st_write(crowns_metrics, 
         "C:/Users/User/Desktop/RandomForest3/sfmonly_filtered_crowns_with_all_metrics.gpkg",
         delete_layer = TRUE)
--------------
crowns_metrics
...
fieldpoints_clean <- fieldpoints %>%
  mutate(
    #SFM = str_trim(SFMonly),                 # remove spaces
    SFMonly = str_replace_all(SFMonly, "[\r\n]", "")  # remove carriage returns / newlines
  ) %>%
  dplyr::rename(treeID = SFMonly) %>%
  as_tibble()   # force into tibble so select/join works

fieldpoints_clean <- fieldpoints_clean %>%
  filter(treecbh > 0, dbh_cm > 0) %>%
  drop_na(treecbh, dbh_cm)
