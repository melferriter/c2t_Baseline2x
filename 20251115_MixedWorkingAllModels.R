...
for (flight in names(files)) {
  ...
  crown_data <- crowns_final %>%
    ...
    mutate(
      ...
      crown_depth    = pmax(lidar_height - treecbh, 0)
    ) %>%
    drop_na()
  ...
  oos_loglog_seq <- collect_loglog_oos(
    filter(crown_data, is_sequoia == 1, crown_depth > 0),
    log(dbh_cm) ~ log(lidar_height) +
      log(pmax(crown_area_m2, 0.01)) +
      log(pmax(crown_depth,    0.01)),
...
fig_residuals <- ggplot(
  preds_all_collapsed %>% filter(Flight == "LiDAR"),
...
crown_metrics_full <- crown_metrics_full %>% 
  left_join(
    crowns_full %>% st_drop_geometry() %>% select(treeID, lidar_height),
    by = "treeID"
  )
...
crown_data_full$treecbh <- 0

# If lidar_height.y exists, use it
if ("lidar_height.y" %in% names(crown_data_full)) {
  crown_data_full$lidar_height <- crown_data_full$lidar_height.y
}

# If lidar_height.x exists, drop it
crown_data_full <- crown_data_full %>%
  select(-matches("lidar_height\\.x$"),
         -matches("lidar_height\\.y$"))


good <- complete.cases(crown_data_full)
preds <- predict(rf_base, newdata = crown_data_full[good, ])

pred_table <- tibble(
  treeID = crown_metrics_full$treeID[good],
  pred_DBH_cm = preds
)
...
ggplot() +
  geom_sf(data = crowns_full, aes(fill = pred_DBH_cm.y), color = NA) +
  scale_fill_viridis_c(name="Predicted DBH (cm)", option="magma", na.value="grey80") +
  theme_bw()
